...
async def test_sse_tts(
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        try:
            ...
            print(
                f"   响应内容: {create_response.text if 'create_response' in locals() else 'N/A'}"
            )
            return False
        except Exception as e:
                        ...
                        try:
                            ...
                            # 任务完成
                                ...
                                if final_status == "succeeded":
                                    ...
                                    print(
                                        f"   - 音频 URL: {final_data.get('output_audio_url')}"
                                    )
                                    print(
                                        f"   - 消耗积分: {final_data.get('cost_credits')}"
                                    )
                                    print(
                                        f"   - 文本字节: {final_data.get('text_utf8_bytes')}"
                                    )
                                else:
                                    print("❌ 任务失败")
...
async def main():
    ...
    if token:
        ...
        if choice == "1":
            # 邮箱密码登录
            email = (
                input("邮箱 (默认: admin@autogame.ai): ").strip() or "admin@autogame.ai"
            )
