...
def utcnow() -> datetime:
    # V1：统一用 UTC naive datetime（后续可换成 timezone-aware）
...
class TxType(str, enum.Enum):
    ...
    admin_adjust = "admin_adjust"  # 管理员手动调账（V1 代替充值）
...
class User(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)  # 创建时间
...
class CreditAccount(Base):
    ...
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)  # 最近更新时间
